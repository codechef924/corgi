class slide {
  /*
    Анимация плавного появления елемента и скрытия (это надо писать в js потому что ты не можешь в scss узнать высоту элемента и приходиться хардкодить)
    используеться для спойлера и select

    target - елемент к которому примениться анимация
    duration - скольку будет идти анимация
  */
  ...
  _slideUp() {
    if (!this.target.classList.contains('_slide')) {
      this.target.classList.add('_slide');
      ...
      window.setTimeout(() => {
        ...
        this.target.classList.remove('_slide');
      }, this.duration);
    }
  }
  _slideDown() {
